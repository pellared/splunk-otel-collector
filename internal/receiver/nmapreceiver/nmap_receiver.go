...
func (nr *nmapReceiver) Start(ctx context.Context, host component.Host) error {
	...
	nr.converter.Start()
	outChan := nr.converter.OutChannel()
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				var in *nmap.Run
				err := xml.Unmarshal([]byte(testdata), &in)
				if err != nil {
					nr.logger.Error("Failed to unmarshal nmap xml", zap.Error(err))
				}
				e := &entry.Entry{
					Body: in,
				...
				nr.converter.Batch([]*entry.Entry{e})
			case <-ctx.Done():
				return
			case out := <-outChan:
				nr.nextConsumer.ConsumeLogs(ctx, out)
			}
		}
	}()

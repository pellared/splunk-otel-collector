...
// contents of the config dir, acting as a log consumer to determining
// which of the underylying receivers were successfully discovered by the
...
func newDiscoverer(logger *zap.Logger) (*discoverer, error) {
	...
	if d, ok := os.LookupEnv("SPLUNK_DISCOVERY_DURATION"); ok {
...
func (d *discoverer) discover(cfg *Config) (map[string]any, error) {
	...
	for observerID, observer := range discoveryObservers {
		d.logger.Debug(fmt.Sprintf("starting %s", observerID.String()))
		if e := observer.Start(context.Background(), d); e != nil {
			d.logger.Info(
	...
	for receiverID, receiver := range discoveryReceivers {
		d.logger.Debug(fmt.Sprintf("starting %s", receiverID.String()))
		if err = receiver.Start(context.Background(), d); err != nil {
			return nil, err
		}
	}
	...
	for receiverID, receiver := range discoveryReceivers {
		if e := receiver.Shutdown(context.Background()); e != nil {
			d.logger.Warn(fmt.Sprintf("error shutting down %s", receiverID.String()), zap.Error(e))

		}
	}
	for observerID, observer := range discoveryObservers {
		if e := observer.Shutdown(context.Background()); e != nil {
			d.logger.Warn(fmt.Sprintf("error shutting down %s", observerID.String()), zap.Error(e))
...
func (d *discoverer) createDiscoveryReceiversAndObservers(cfg *Config) (map[config.ComponentID]component.LogsReceiver, map[config.ComponentID]component.Extension, error) {
	...
	// Without using the expand converter, env var support isn't available in discovery.yaml items.
	// Its converted config values will be used by the underlying components but final discovery config
	// values should be in pre-conversion form (TODO).
	ec := expandconverter.New()
	...
	for _, observerID := range cfg.observersForDiscoveryMode() {
		...
		// defaultObserverConfig := observerFactory.CreateDefaultConfig()
		// observerConfig := defaultObserverConfig.(*dockerobserver.Config)
		...
		if err = ec.Convert(context.Background(), observerCfgMap); err != nil {
			return nil, nil, fmt.Errorf("error converting environment variables in %q config: %w", observerID.String(), err)
		}
		if err = observerCfgMap.Unmarshal(observerConfig); err != nil {
			return nil, nil, fmt.Errorf("failed unmarshaling %s config: %w", observerID.String(), err)
		}
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed creating %s extension: %w", observerID.String(), err)
		}

		// TODO: determine why this was here
		// observerConfigMap := map[string]any{}
		// if err = mapstructure.Decode(defaultObserverConfig, &observerConfigMap); err != nil {
		// 	return nil, nil, fmt.Errorf("failed converting observer %s config to confmap: %w", observerID.String(), err)
		// }
		...
		for receiverID, receiver := range cfg.ReceiversToDiscover {
			if observerRule, hasRule := receiver.Rule[observerID]; !hasRule {
				d.logger.Info(fmt.Sprintf("disregarding %s without a %s rule", receiverID.String(), observerID.String()))
				continue
			} else {
			...
			if !hasObserverConfigBlock && !hasDefault {
				d.logger.Info(fmt.Sprintf("disregarding %s without a default and %s config", receiverID.String(), observerID.String()))
				continue
			}
			if hasObserverConfigBlock {
				if hasDefault {
					if err = mergeMaps(defaultConfig, observerConfigBlock); err != nil {
						return nil, nil, fmt.Errorf("failed merging %s config for %s: %w", receiverID.String(), observerID.String(), err)
					}
				} else {
		...
		if err = ec.Convert(context.Background(), discoveryReceiverConfMap); err != nil {
		...
		dCfg.Merge(confmap.NewFromStringMap(map[string]any{
			"service": map[string]any{
				"pipelines": map[string]any{
					...
					// TODO: support logs in receiver creator
	...
	for i := 0; i < rlogs.Len(); i++ {
		...
		if obsID != "" {
			if observerID, err = config.NewComponentIDFromString(obsID); err != nil {
				d.logger.Debug(fmt.Sprintf("invalid %s", discovery.ObserverIDAttr), zap.Error(err))
				continue
			}
		}
			...
			for k := 0; k < lrs.Len(); k++ {
				...
				if currentReceiverStatus != discovery.Successful || currentObserverStatus != discovery.Successful {
					if rStatusAttr, ok := lr.Attributes().Get(discovery.StatusAttr); ok {
						...
						observerStatus := determineCurrentStatus(currentObserverStatus, rStatus)
						d.discoveredReceivers[receiverID] = receiverStatus
						d.discoveredObservers[observerID] = observerStatus
					}
				}
			}
		}
